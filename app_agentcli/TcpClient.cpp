...
TcpClient::TcpClient(boost::asio::io_service &io_ser) :
	m_ioSvr(io_ser),
	m_socket(io_ser),
	m_timerHeartbeat(io_ser, boost::posix_time::seconds(HEARTBEAT_INTERV)), 
	m_writeImpl2(std::bind(&TcpClient::writeImpl2, this, placeholders::_1))
{
}
...
bool TcpClient::connect(string &errmsg)
{
	...
	m_readHeader = true;
	boost::asio::async_read(m_socket, m_streambuf, boost::asio::transfer_exactly(2), 
	...
	if (!err)
	{
		...
		if (m_readHeader)
		{
			if (m_streambuf.size() >= 2)
			{
				...
				//m_recvTotal += 2;
				m_readHeader = false;
				boost::asio::async_read(m_socket, m_streambuf, boost::asio::transfer_exactly(m_lenBody), 
					boost::bind(&TcpClient::handle_read, this, boost::asio::placeholders::error, 
						boost::asio::placeholders::bytes_transferred));
			}
		}
...
void TcpClient::toHeartbeat(const boost::system::error_code& errcode)
{
	if (!errcode){
		...
		boost::asio::async_write(m_socket, boost::asio::buffer(*m_msgHeartbeat),
